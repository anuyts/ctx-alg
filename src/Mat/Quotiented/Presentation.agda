...
module EqTheory {sign : Signature} (presF : PresentationF sign) where
  ...
  module _ (eqTheory : EqTheory) where
    ...
    data Term (X : MType) : (sort : Sort) → Type where
      ...
      byAxiom : ∀ {sort} → (axiom : Axiom sort) → (f : ∀ sort' → mtyp (msetArity axiom) sort' → TermF X sort')
        → term (joinTermF sort (mapTermF f sort (lhs axiom)))
         ≡ term (joinTermF sort (mapTermF f sort (rhs axiom)))
      ast : ∀ {sort} → Term1 (Term X) sort → Term X sort
      ast-term :  ∀ {sort} → (t : Term1 (TermF X) sort) → ast (mapTerm1 (λ _ → term) sort t) ≡ term (astF t)
    ...
    joinTerm sort (term t) = joinTermFTerm sort t
    joinTerm sort (byAxiom axiom f i) = {!!} -- I think this is false!
    joinTerm sort (ast (term1 o args)) = ast (term1 o λ p → joinTerm (arity o ! p) (args p))
    joinTerm sort (ast-term (term1 o args) i) = ast (term1 o λ p → joinTermFTerm (arity o ! p) (args p))
    joinTerm sort (isSetTerm t1 t2 et et' i j) = isSetTerm
      (joinTerm sort t1)
      (joinTerm sort t2)
      (cong (joinTerm sort) et)
      (cong (joinTerm sort) et') i j
